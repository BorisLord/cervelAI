...
# Fetches the project tarball, then runs phase 1 (cervelAI-lxc.sh).
...
[[ $EUID -eq 0 ]] || {
    echo "cervelAI: run as root on the Proxmox host" >&2
    exit 1
}
command -v pct >/dev/null || {
    echo "cervelAI: 'pct' not found, not a Proxmox host?" >&2
    exit 1
}
...
[[ -f "$workdir/cervelAI-lxc.sh" ]] ||
    {
        echo "cervelAI: unexpected archive (cervelAI-lxc.sh missing), check CERVELAI_REPO/CERVELAI_REF" >&2
        exit 1
    }
