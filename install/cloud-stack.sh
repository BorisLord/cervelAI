...
install_cloud_stack_scaleway() { mise_use "scaleway" latest scw; }
...
install_cloud_stack_opentofu() { mise_use "opentofu" latest tofu; }
install_cloud_stack_pulumi() { mise_use "pulumi"; }

install_cloud_stack_all() {
    _dispatch_csv cloud-stack CERVELAI_CLOUD_STACK \
        "aws,flyctl,cloudflared,supabase,doctl,hcloud,scaleway,gcloud,azure,opentofu,pulumi"
}
