...
install_shell_oh_my_zsh() {
    ...
    if [[ -d "/home/$u/.oh-my-zsh" ]]; then
        log_skip "oh-my-zsh already installed for $u"; return 0
    fi
...
install_shell_bash_it() {
    ...
    if [[ -d "/home/$u/.bash_it" ]]; then
        log_skip "bash-it already installed for $u"; return 0
    fi
...
install_shell_all() {
    case "${CERVELAI_SHELL:-bash}" in
        bash) install_shell_bash; install_shell_bash_it ;;
        zsh)  install_shell_bash; install_shell_zsh; install_shell_oh_my_zsh ;;
        fish) install_shell_bash; install_shell_fish ;;
        none) log_skip "shell: none" ;;
        *)    log_warn "unknown CERVELAI_SHELL=${CERVELAI_SHELL}, defaulting to bash"
              install_shell_bash; install_shell_bash_it ;;
    esac
