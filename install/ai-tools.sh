...
# ttok dropped 2026-05 — stale since 2024 (v0.3). Use `llm tokens` (built-in command in simonw/llm).
...
install_ai_tools_all() {
    _dispatch_csv ai-tools CERVELAI_AI_TOOLS \
        "markitdown,fabric,mcp-inspector,code2prompt,llm,shell-gpt,gptscript,aichat,agents,promptfoo"
}
